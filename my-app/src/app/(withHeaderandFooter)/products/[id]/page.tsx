...
export default function ProductInfo() {
    ...
    return (
        <>
                ...
                <div className="flex flex-col sm:flex-row justify-between gap-16 mx-0 sm:mx-72">
                    <div className="flex flex-col gap-10 px-10 sm:px-0">
                        ...
                        <div className="flex flex-row gap-4">
                            <button className={`flex items-center gap-4 justify-center h-12 hover:bg-[#A35C7A] text-[1rem] text-gray-50 uppercase bg-[#C890A7] py-2 w-full ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            ...
                            <button className={`flex items-center gap-4 justify-center hover:bg-[#E16A54] text-[1rem] uppercase text-gray-50 bg-[#DF9755] py-2 w-full ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    ...
                    <div className="flex flex-col w-full h-auto sm:max-h-[47rem] overflow-y-auto bg-gray-100 custom-scrollbar">
                        <div className="py-6 bg-white">
                            ...
                            <p className="text-green-600 font-semibold mt-3">Special price</p>
                            <div className="flex items-center text-2xl font-semibold">
                                <span className="mr-1">₹</span>{product?.price} <span className="text-gray-500 text-sm line-through ml-2">₹4,999</span> <span className="text-green-600 text-sm ml-2">85% off</span>
                            </div>
                            <div className="flex items-center mt-4">
                                {product && (
                                    <div className="bg-green-600 text-white px-2 py-1 rounded text-sm">{product.rating.rate}★</div>
                                )}
                                <span className="text-gray-600 ml-2 text-sm">{product?.rating.count} reviews</span>
                                <span className="text-blue-600 ml-2 text-sm">Assured</span>
                            </div>
                            ...
                            <div className="mt-4">
                                ...
                                <div className="flex space-x-2">
                                    {['S', 'M', 'L', 'XL', 'XXL'].map((size) => (
                                        <button
                                            key={size}
                                            className="border border-gray-300 px-4 py-2 rounded hover:border-blue-500 focus:border-blue-500"
                                        >
                                            {size}
                                        </button>
                                    ))}
                                    ...
                                    <div className="flex space-x-2">
                                        {['#000000', '#D39D55', '#8EB486', '#F9C0AB'].map((color) => (
                                            <button
                                                ...
                                                className="w-8 h-8 rounded-full border border-gray-300 hover:border-blue-500 focus:border-blue-500"
                                            ></button>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="mt-4 flex flex-col gap-4">
                                <p className="font-semibold flex items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#8EB486" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-map-pinned"><path d="M18 8c0 3.613-3.869 7.429-5.393 8.795a1 1 0 0 1-1.214 0C9.87 15.429 6 11.613 6 8a6 6 0 0 1 12 0" /><circle cx="12" cy="8" r="2" /><path d="M8.714 14h-3.71a1 1 0 0 0-.948.683l-2.004 6A1 1 0 0 0 3 22h18a1 1 0 0 0 .948-1.316l-2-6a1 1 0 0 0-.949-.684h-3.712" /></svg>
                                    Deliver to
                                </p>
                            ...
                            <p className="text-gray-600 my-4 font-semibold flex items-center gap-2">
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-circle-help"><circle cx="12" cy="12" r="10" /><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3" /><path d="M12 17h.01" /></svg>
                            </p>


                            <div className="mt-4">
                                <p className="font-medium text-[0.8rem] flex items-center gap-1">Delivery by 19 Jan, Sunday <span className="text-[1.17rem] text-gray-300 font-light ml-2">|</span> <span className="text-green-600 text-[1.16rem] ml-2 mr-1">Free</span> <span className="text-[1rem]">₹40</span> </p>
                                <button className="text-blue-600 text-sm mt-1">View Details</button>
                            </div>

                            <div className="mt-4 flex items-start gap-16 text-gray-400">
                                Seller
                                <div className="flex flex-col items-start gap-1">
                                    <p className="text-blue-600">KLOSIA <span className="bg-blue-600 text-white px-2 rounded-full text-sm ml-2">3.8★</span></p>
                                    <p className="text-gray-600 text-sm mt-1">10 Days Return Policy</p>
                                    <button className="text-blue-600 text-sm">See other sellers</button>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white">
                            <div className="flex items-center justify-between border-b py-4 px-2">
                                <div className="flex flex-col items-start gap-2">
                                    ...
                                    <div className="flex items-center">
                                        <div className="bg-green-600 text-white px-3 py-1 rounded-full text-sm">{product?.rating.rate}★</div>
                                        <span className="text-gray-600 ml-2 text-sm">{product?.rating.count} reviews</span>
                                    </div>
                                </div>
                                <button className="bg-blue-500 text-white px-4 py-1 rounded text-sm">Rate Product</button>
                            </div>
                                ...
                                {reviews.map((review, index) => (
                                    <div key={index} className="border-b pb-6">
                                        <div className="flex items-center space-x-2">
                                            <div className="bg-green-600 text-white px-3 py-[0.15rem] rounded-full text-sm">{review.rating}★</div>
                                            <p>{review.review}</p>
                                        </div>
                            ...
                            <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between">
                                ...
                                <button className="bg-blue-500 text-white px-4 py-1 rounded w-[50%] sm:w-auto">Post Your Question</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section className="flex flex-col justify-between m-6 py-4 px-6 sm:mx-72 bg-gray-100 gap-6">
                <h2 className="text-[1.4rem] text-[#AB4459] font-normal uppercase">Similar Products</h2>
                <div className="grid gris-cols-1 sm:grid-cols-4 gap-4 mt-6">
                    {loading ? (
