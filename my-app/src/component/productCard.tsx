...
export default function ProductCard({ product }: { product: { id: number; title: string; description: string; category: string; price: number; image: string; rating: { rate: number; count: number } } }) {
    ...
    return (
        <>
            <div className="bg-[#E5E3D4] flex flex-col gap-1 rounded-2xl cursor-pointer hover:shadow-2xl transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-105 duration-300">
                <div className="relative h-[15rem] md:h-[12rem] lg:h-[15rem] bg-gray-50 rounded-t-2xl group">
                    ...
                    <Link href={`/products/${product.id}`} className="absolute bottom-2 right-2 bg-gray-200 text-gray-50 p-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <Expand size={20} className='text-gray-800' />
                    </Link>
                </div>
                <div className="flex flex-col justify-between h-[10rem] md:h-[8rem] lg:h-[10rem] px-3 pt-1 pb-2">
                    <div className='flex flex-col gap-1'>
                        <div className="flex flex-row justify-between items-start">
                            <div className="flex flex-col">
                                <span className="text-lg md:text-md lg:text-xl font-medium">
                                    {product.title.slice(0, 20)}
                                </span>
                            </div>
                        </div>
                        <div className="flex flex-row">
                            <span className="text-sm md:text-xs lg:text-base text-gray-500"><strong className='font-semibold'>Category: </strong>{product.category}</span>
                        </div>
                        <div className="flex flex-row items-start justify-between">
                            <h2 className="flex flex-col items-start text-sm md:text-xs lg:text-base text-gray-500">
                                <strong className='font-medium mr-2 text-[0.9rem]'>Rating: </strong>
                                <div className='flex flex-row'>
                            ...
                            <span className="bg-blue-100 text-blue-800 font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-3 text-xs md:text-[0.75rem] lg:text-sm">
                                {product.rating?.count} reviews
                            </span>
                        </div>
                    </div>
                    <div className="flex flex-row justify-between items-center gap-4">
                        <span className="font-medium text-red-600 text-lg md:text-md lg:text-[1.5rem]"> ${product.price} </span>
                        <div className="group relative flex justify-center items-center text-zinc-900 text-sm font-bold">
                            <div className="shadow-md flex items-center group-hover:gap-2 bg-yellow-300 p-2 rounded-full cursor-pointer duration-300">
                                ...
                                <span className="text-[0px] group-hover:text-xs md:group-hover:text-sm font-medium duration-300">Add to Cart</span>
                            </div>
                        </div>
                    </div>
                </div>
